...
def test_point_inside_rounded_rect():
    ...
    assert Qlementine.isPointInRoundedRect(center, rect, 10.0) is True


def test_point_outside_rounded_rect():
    rect = QtCore.QRectF(0, 0, 100, 100)
    outside = QtCore.QPointF(200, 200)
    assert Qlementine.isPointInRoundedRect(outside, rect, 10.0) is False
...
def test_point_in_corner_excluded_by_radius():
    ...
    assert Qlementine.isPointInRoundedRect(corner, rect, 50.0) is False
